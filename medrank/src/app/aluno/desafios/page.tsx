...
export default async function DesafiosAlunoPage() {
  ...
  if (usesDemoStore()) {
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <h1 className="mt-4 text-2xl font-bold text-slate-900">Desafios e gamificação</h1>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl bg-white p-4 text-slate-900 ring-1 ring-slate-200"><p className="text-sm text-slate-600">Maior sequência</p><p className="text-2xl font-bold text-slate-900">14 dias</p></div>
        </div>
        <div className="mt-8 space-y-4">
          {challenges.map(({ challenge, currentValue, completed, description }) => (
            <div key={challenge.id} className={`rounded-xl p-5 ring-1 ${completed ? 'bg-emerald-50 ring-emerald-200' : 'bg-white ring-slate-200'}`}>
              ...
              <div className="mt-4"><div className="flex justify-between text-sm"><span>Progresso</span><span>{currentValue} / {challenge.target_value}</span></div><div className="mt-2 h-3 rounded-full bg-slate-100"><div className="h-3 rounded-full bg-emerald-500" style={{ width: `${Math.min(100, (currentValue / challenge.target_value) * 100)}%` }} /></div><p className="mt-2 text-xs text-slate-600">Recompensa: +{challenge.bonus_points} pontos</p></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      {streak && (
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl bg-white p-4 text-slate-900 ring-1 ring-slate-200">
            <p className="text-sm text-slate-600">Maior sequência</p>
            <p className="text-2xl font-bold text-slate-900">{streak.longest_streak} dias</p>
          </div>
        </div>
      )}
      ...
      <div className="mt-8 space-y-4">
        {challenges.length === 0 ? (
          <p className="text-slate-600">Nenhum desafio ativo esta semana.</p>
        ) : (
          challenges.map(({ challenge, currentValue, completed, description }) => (
            <div
              key={challenge.id}
            ...
            >
              ...
              <div className="mt-4">
                ...
                <p className="mt-2 text-xs text-slate-600">Recompensa: +{challenge.bonus_points} pontos</p>
              </div>
            </div>
          ))
        )}
