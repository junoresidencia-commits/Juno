...
export default async function RelatoriosPage({
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          <section className="rounded-xl bg-white p-5 text-slate-900 shadow-sm ring-1 ring-slate-200">
            <h2 className="font-semibold">Estatísticas por tema</h2>
            <p className="text-xs text-slate-600">Período ranking: {bounds.label}</p>
          ...
          <section className="rounded-xl bg-white p-5 text-slate-900 shadow-sm ring-1 ring-slate-200">
            ...
            <div className="mt-4 space-y-3">{questions.map((q) => <div key={q.id} className="border-b border-slate-100 pb-3 last:border-0"><p className="line-clamp-2 text-sm">{q.enunciado}</p><p className="mt-1 text-xs text-slate-600">{q.tema} · {q.origem} · {q.erros}/{q.totalRespostas} erros ({formatPercent(q.taxaErro)})</p></div>)}</div>
          </section>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <section className="mt-8 rounded-xl bg-white p-5 text-slate-900 shadow-sm ring-1 ring-slate-200">
