  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-8">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl bg-white p-4 text-slate-900 shadow-sm ring-1 ring-slate-200">
          <p className="text-sm text-slate-600">Para revisar</p>
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2">
        <section className="rounded-2xl bg-white p-6 text-slate-900 shadow-sm ring-1 ring-slate-200">
          ...
          <ol className="mt-4 space-y-2">
            {ranking.length === 0 ? (
              <p className="text-sm text-slate-600">Faça um simulado para entrar no ranking.</p>
            ) : (
              ranking.map((r) => (
        ...
        <section className="rounded-2xl bg-white p-6 text-slate-900 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="mt-4 space-y-3">
            {history.length === 0 ? (
              <p className="text-sm text-slate-600">Nenhum simulado finalizado ainda.</p>
            ) : (
              history.map((s) => (
                <Link
                  key={s.id}
