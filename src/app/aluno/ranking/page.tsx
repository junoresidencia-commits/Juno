...
export default async function RankingAlunoPage({
  ...
  const dailyRankingDate = yesterdayDate;

  if (isSkipAuth()) {
    const { rankings } = period === 'daily'
      ? getDemoRanking('daily', dailyRankingDate)
      ...
      <div className="mx-auto max-w-lg px-4 py-6">
        ...
        {period === 'daily' && (
          <p className="mt-3 text-sm text-slate-500">
            {studentDailyRankingLabel(dailyRankingDate)}. O de hoje sai amanhã.
          </p>
        )}
        {myRanking && (
        ...
        <ol className="mt-6 space-y-2">
          {rankings.length === 0 ? (
            <li className="text-sm text-slate-500">{studentRankingAvailableTomorrowMessage()}</li>
          ) : (
            rankings.map((r) => {
  ...
  const bounds = period === 'daily'
    ? { start: dailyRankingDate, end: dailyRankingDate }
